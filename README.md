...
> **`man.ua`** — це відкритий репозиторій та довідковий центр перекладу, стандартизації та системної інтеграції понад **34 000 сторінок документації Linux man-pages** українською мовою.

---

## ⚡ Швидке встановлення після клонування (1 команда)

Якщо ви щойно схилили репозиторій до себе на комп'ютер, встановіть усі розділи в систему однією командою:

```bash
git clone https://github.com/AndreyVeremchuck/man.ua.git
cd man.ua

# Автоматичне встановлення всіх розділів (man1–man9) та оновлення бази mandb:
sudo ./install.sh
...
Усі вихідні посібники структуровано за стандартними розділами системи:
...
├── man9/                    # 1 файл: Внутрішні функції та процедури ядра Linux
├── assets/                  # Скріншоти та візуалізації інтерфейсу
...
├── Makefile                 # Класичне встановлення через make install / uninstall
├── generate_screenshots.py  # Генератор термінальних скріншотів високої чіткості
├── README.md                # Головний посібник користувача та опис розділів
...
# 3. Встановлення пакетів репозиторію man.ua:
sudo ./install.sh
```
...
2. Додайте або покращіть переклад man-сторінки у відповідний розділ (`man1/` ... `man9/`).
